...
class ProductsViewModel @Inject constructor(
            ...
            when (position) {
                0 -> {
                    productsResponse.value =
                        NetworkResult.Success(productsResponse.value?.data!!.sortedBy { it.description })
                }
                1 -> {
                    productsResponse.value =
                        NetworkResult.Success(productsResponse.value?.data!!.sortedBy { it.price })
                }
                2 -> {
                    productsResponse.value =
                        NetworkResult.Success(productsResponse.value?.data!!.sortedByDescending { it.price })
                }
            }
        }
    ...
    fun addProductToWishlistForUser(productId: Long) = viewModelScope.launch {
        ...
        addProductToWishlistResponse.value =
            wishlistManager.addProductToWishlistForUserSafeCall(productId)
    }
    ...
    fun removeProductFromWishlistForUser(productId: Long) = viewModelScope.launch {
        ...
        removeProductFromWishlistResponse.value =
            wishlistManager.removeProductFromWishlistForUserSafeCall(productId)
    }
    ...
    private suspend fun getProductsSafeCall(categoryId: Long) {
        ...
        if (Utils.hasInternetConnection(getApplication<Application>())) {
            try {
                val response =
                    repository.remote.getProductsByCategory(categoryId, loginManager.readUserId())
    ...
    private suspend fun getProductsInPriceRangeSafeCall(dto: FilterProductDto) {
        ...
        if (Utils.hasInternetConnection(getApplication<Application>())) {
            try {
                val response =
                    repository.remote.getFilteredProductsForCategory(dto, loginManager.readUserId())
                productsResponse.value = handleProductsResponse(response)
        ...
        if (Utils.hasInternetConnection(getApplication<Application>())) {
            try {
                withContext(Dispatchers.Main) {
                    val response =
                        repository.remote.getSearchedProducts(searchText, loginManager.readUserId())
